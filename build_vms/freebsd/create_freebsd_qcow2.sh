...
if [ ${dryrun} -eq 0 ] && [ ${options_valid} -eq 0 ]; then
       ...
       echo "[DRYRUN] FreeBSD: ${FREEBSD_MAJOR}.${FREEBSD_MINOR}" 1>&2
...
if [ -f /etc/os-release ] && [ ${options_valid} -eq 0 ]; then
   ...
   if [ -n "${PACKER_CMD}" ]; then
	  ...
	  adminuser_passwd_enc="$(echo $DEFAULT_PASSWORD | openssl passwd -6 -salt $(openssl rand -base64 16) --stdin)";
	  root_passwd_enc="$(echo $DEFAULT_PASSWORD | openssl passwd -6 -salt $(openssl rand -base64 16)  --stdin)";
	  # setup pkrvars file for run.
      fi;
      echo "hostname=\"${hostname}\"" > "${pkrvars_name}";
      echo "vm_name=\"${packer_basename}srv-${FREEBSD_MAJOR}${FREEBSD_MINOR}\"" >> "${pkrvars_name}";
	  ...
	  if [ ${dryrun} -eq 0 ]; then
	     ...
	     rm -r -f output-artifacts;
	     packer build -force -var "adminuser_password_enc=${adminuser_passwd_enc}" \
