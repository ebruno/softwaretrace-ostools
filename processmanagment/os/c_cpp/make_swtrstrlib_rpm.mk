...
SWTRSTRLIB_SRCDIR_NAME   = ${STRWSRTLIB_NAME}-${SWTRSTRLIB_MAJOR}.${SWTRSTRLIB_MINOR}.${SWTRSTRLIB_SUBVERSION}.${SWTRSTRLIB_PRIVATE_BUILD_ID}.${SWTRSTRLIB_BUILD_NUMBER}
...
swtrstrlib_create_src:
	...
	@if test -d "${STRWSRTLIB_NAME}.${SWTRSTRLIB_MAJOR}.${SWTRSTRLIB_MINOR}.${SWTRSTRLIB_SUBVERSION}"; then \
	   rm -r -f "${STRWSRTLIB_NAME}.${SWTRSTRLIB_MAJOR}.${SWTRSTRLIB_MINOR}.${SWTRSTRLIB_SUBVERSION}"; \
	fi;
...
swtrstrlib_create_rpm:
	...
	@if test -d ${CURDIR}/${SPEC_TEMPLATE_DIR}; then \
		cat ${SPEC_TEMPLATE_DIR}/${STRWSRTLIB_NAME}.spec.tmpl | sed -e "s/XMAJOR/${SWTRSTRLIB_MAJOR}/g" > ${SPEC_DIR}/${STRWSRTLIB_NAME}.spec; \
		sed -e "s/XMINOR/${SWTRSTRLIB_MINOR}/g" -i ${SPEC_DIR}/${STRWSRTLIB_NAME}.spec;\
		sed -e "s/XSUBVERSION/${SWTRSTRLIB_SUBVERSION}/g" -i ${SPEC_DIR}/${STRWSRTLIB_NAME}.spec;\
		sed -e "s/XPRIVATE_BUILD_ID/${SWTRSTRLIB_PRIVATE_BUILD_ID}/g" -i ${SPEC_DIR}/${STRWSRTLIB_NAME}.spec;\
		sed -e "s/XBUILD_NUMBER/${SWTRSTRLIB_BUILD_NUMBER}/g" -i ${SPEC_DIR}/${STRWSRTLIB_NAME}.spec;\
		sed -e "s/XRELEASE_KEY/${SWTRSTRLIB_RELEASE_KEY}/g" -i ${SPEC_DIR}/${STRWSRTLIB_NAME}.spec;\
#		rpmbuild %(RPM_VERBOSE} -bb ${SPEC_DIR}/${STRWSRTLIB_NAME}.spec; \
	else \
