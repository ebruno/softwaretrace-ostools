...
*/
int swtrprcmgt_get_process_stat(SWTPROC_MGT *ctrl, pid_t pid, SWTPROC_STAT_INFO *proc_info) {
  bool continue_processing = false;   
  char *field_start = NULL;
  char *tmp_ptr = NULL;
  char comm[SWTRPOCMGT_SMALL_WRKBUF+1] = "";
  char cur_state = '\0'; 
  char stat_file_pattern[SWTRPOCMGT_SMALL_WRKBUF+1] = "%s/%d/stat";
  ...
  if (ctrl != NULL) {
	...
	if ((ctrl->mgt.v1.kernel_major >= SWTRPOCMGT_KERNEL_MAJOR_V3) && (ctrl->mgt.v1.kernel_minor >= SWTRPOCMGT_KERNEL_MINOR_V5)) {
	  ...
	  count_fields = sscanf(stat_info,SWTRPOCMGT_STAT_FMT,
		 ...
		 &proc_info->stat.kernel_3_5.comm,
	  ...
	  sscanf(stat_info,SWTRPOCMGT_STAT_FMT_KV33,
		 ...
		 &proc_info->stat.kernel_3_3.comm,
	  ...
	  sscanf(stat_info,SWTRPOCMGT_STAT_FMT_PRE_KV33,
		 ...
		 &proc_info->stat.kernel_2_6.comm,
